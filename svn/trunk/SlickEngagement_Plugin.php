...
/*If there's a featured image, show it*/
        ...
        if ( is_array($response) ) {
          ...
          if (!empty($response_text)) {
            ...
            if ( !empty($filmstrip_config) || !empty($dcm_config) ) {
              ...
              echo '"use strict";(async(e,t)=>{const n=e?JSON.parse(e):null;const r=t?JSON.parse(t):null;if(n||r){const e=async()=>{if(document.body){if(n){s(n.selector,n.position||"after selector","slick-film-strip",72)}if(r){r.forEach((e=>{if(e.selector){s(e.selector,e.position||"after selector","slick-inline-search-panel",350,e.id)}}))}return}window.requestAnimationFrame(e)};window.requestAnimationFrame(e)}const c=async(e,t)=>{const n=Date.now();while(true){const r=document.querySelector(e);if(r){return r}const c=Date.now();if(c-n>=t){throw new Error("Timeout")}await o(200)}};const o=async e=>new Promise((t=>{setTimeout(t,e)}));const s=async(e,t,n,r,o)=>{try{const s=await c(e,5e3);const i=o?document.querySelector(`.${n}[data-config="${o}"]`):document.querySelector(`.${n}`);if(s&&!i){const e=document.createElement("div");e.style.minHeight=r+"px";e.classList.add(n);if(o){e.dataset.config=o}switch(t){case"after selector":s.insertAdjacentElement("afterend",e);break;case"before selector":s.insertAdjacentElement("beforebegin",e);break;case"first child of selector":s.insertAdjacentElement("afterbegin",e);break;case"last child of selector":s.insertAdjacentElement("beforeend",e);break}return e}}catch(t){console.log("plugin","error",`Failed to inject ${n} for selector ${e}`)}return false}})' . "\n";
    ...
    public function addSlickPageHeader()
    {
        ...
        if ($siteCode) {
            ...
            echo '"use strict";(async(e,t)=>{if(location.search.indexOf("no-slick")>=0){return}let o;const c=()=>(performance||Date).now();let a=window.$slickBoot=window.$slickBoot||{};a.rt=e;a._es=c();a.ev="2.0.1";a.l=async(e,t)=>{try{let a=0;if(!o&&"caches"in self){o=await caches.open("slickstream-code")}if(o){let i=await o.match(e);if(!i){a=c();await o.add(e);i=await o.match(e);if(i&&!i.ok){i=undefined;o.delete(e)}}if(i){const e=i.headers.get("x-slickstream-consent");return{t:a,d:t?await i.blob():await i.json(),c:e||"na"}}}}catch(e){console.log(e)}return{}};const i=e=>new Request(e,{cache:"no-store"});if(!a.d){const o=i(`${e}/d/page-boot-data?site=${t}&url=${encodeURIComponent(location.href.split("#")[0])}`);let{t:n,d:s,c:l}=await a.l(o);if(s){if(s.bestBy<Date.now()){s=undefined}else if(n){a._bd=n;a.c=l}}if(!s){a._bd=c();const e=await fetch(o);const t=e.headers.get("x-slickstream-consent");a.c=t||"na";s=await e.json()}if(s){a.d=s;a.s="embed"}}if(a.d){let e=a.d.bootUrl;const{t:t,d:o}=await a.l(i(e),true);if(o){a.bo=e=URL.createObjectURL(o);if(t){a._bf=t}}else{a._bf=c()}const n=document.createElement("script");n.src=e;document.head.appendChild(n)}else{console.log("[Slick] Boot failed")}})' . "\n";
