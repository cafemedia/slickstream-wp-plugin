...
// Ensure pages can be configured with categories and tags
    ...
    public function doSlickStoryShortcode($attrs, $content, $tag)
    {
        ...
        if ($channelid !== "nochannel" && $storyid !== "nostory") {
            $output .= '  <script async src="https://' . $domain . '/' . $channelid . '/e3/story-embed.js"></script>' . "\n";
            $output .= '  <slick-story-viewer id="story-' . $storyid . '" src="' . $channelid . '/' . $storyid . '"></slick-story-viewer>' . "\n";
        } else {
            $output .= '  <script async src="https://' . $domain . '/' . $channelid . '/e3/story-embed.js"></script>' . "\n";
...
/*If there's a featured image, show it*/
    ...
    public function addSlickPageHeader()
    {
        global $post;
        echo "\n";
        echo '<meta property="slick:wpversion" content="1.1.7" />' . "\n";
