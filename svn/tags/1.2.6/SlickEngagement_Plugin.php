...
/*If there's a featured image, show it*/
    ...
    public function addSlickPageHeader()
    {
        ...
        echo "\n";
        echo '<meta property="slick:wpversion" content="1.2.6" />' . "\n";
        ...
        if ($siteCode) {
            ...
            echo '"use strict";(async(e,t)=>{if(location.search.indexOf("no-slick")>=0){return}let o;const c=()=>(performance||Date).now();const a=window.$slickBoot={rt:e,_es:c(),ev:"2.0.0",l:async(e,t)=>{try{let a=0;if(!o&&"caches"in self){o=await caches.open("slickstream-code")}if(o){let n=await o.match(e);if(!n){a=c();await o.add(e);n=await o.match(e);if(n&&!n.ok){n=undefined;o.delete(e)}}if(n){const e=n.headers.get("x-slickstream-consent");return{t:a,d:t?await n.blob():await n.json(),c:e||"na"}}}}catch(e){console.log(e)}return{}}};const n=e=>new Request(e,{cache:"no-store"});const i=n(`${e}/d/page-boot-data?${innerWidth<=600?"mobile&":""}site=${t}&url=${encodeURIComponent(location.href.split("#")[0])}`);let{t:s,d:l,c:d}=await a.l(i);if(l){if(l.bestBy<Date.now()){l=undefined}else if(s){a._bd=s;a.c=d}}if(!l){a._bd=c();const e=await fetch(i);const t=e.headers.get("x-slickstream-consent");a.c=t||"na";l=await e.json()}if(l){a.d=l;let e=l.bootUrl;const{t:t,d:o}=await a.l(n(e),true);if(o){a.bo=e=URL.createObjectURL(o);if(t){a._bf=t}}else{a._bf=c()}const i=document.createElement("script");i.src=e;document.head.appendChild(i)}else{console.log("[Slick] Boot failed")}})' . "\n";
